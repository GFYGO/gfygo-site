...
async function initDashboard() {
    ...
    try {
        ...
        if (response.ok && data.code === 200) {
            ...
            // now_permission 运行时等级由 token claims 决定（config.js 初始化 window.__nowPermission）
            // 用户直接访问 adminN/dashboard.html 时若 level 不足，需主动点切换按钮（Q10.1 接受的新流程）
...
window.renderPermissionButtons = window.renderPermissionButtons || function renderPermissionButtons(realLevel) {
...
window.handlePermissionClick = window.handlePermissionClick || async function handlePermissionClick(level) {
...
function initDeletion() {
    ...
    if (cancelBtn) cancelBtn.addEventListener('click', closeDeletionModal);
...
async function sendDeletionCode() {
    ...
    if (!email) {
        if (typeof Toast !== 'undefined') Toast.show('请输入注册邮箱', 'error');
        return;
    }
...
async function cancelDeletion() {
    ...
    try {
        ...
        const d = await r.json();
        if (d.code === 200) {
            if (typeof Toast !== 'undefined') Toast.show('注销请求已取消', 'success');
            renderDeletionStatus();
        } else {
            if (typeof Toast !== 'undefined') Toast.show(d.msg || '取消失败', 'error');
        }
    } catch (e) {
        if (typeof Toast !== 'undefined') Toast.show('网络请求失败', 'error');
    }
...
async function renderDeletionStatus() {
    const statusArea = document.getElementById('deletionStatusArea');
    const actionArea = document.getElementById('deletionActionArea');
    ...
    try {
        ...
        if (req.status === 'pending') {
            ...
            // 实际上用户已登录，不应该出现 completed 状态的用户
            statusArea.innerHTML = `
